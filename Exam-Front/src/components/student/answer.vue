  ...
  <div id="answer">
    ...
    border-radius: 4px;
        ...
        <div class="right">
          <div class="title">
            <p>{{title}}</p>
            <i class="iconfont icon-right auto-right ml-auto mr-2.5 text-blue-400"></i>
            <span>全卷共{{topicCount[0] + topicCount[1] + topicCount[2]}}题  <i class="iconfont icon-time"></i>倒计时：<b>{{time}}</b>分钟<b>{{second}}</b>秒</span>
          </div>
          <!-- .right .content {
  @apply bg-white m-3 ml-0 h-3/5
...
-->
          <div class="content bg-white m-3 ml-0 h-3/5">
            <p class="topic py-5 pt-8"><span class="number inline-flex justify-center w-5 h-5 rounded-lg mr-1 bg-blue-300">{{number}}</span>{{showQuestion}}</p>
            <div v-if="currentType == 1">
              <el-radio-group v-model="radio[index]" @change="getChangeLabel" class="flex flex-col">
          ...
          <div class="operation ">
            <ul class="end flex justify-center items-center text-blue-400">
              <li @click="previous()" class="cursor-pointer mx-24"><i class="iconfont icon-previous"></i><span>上一题</span></li>
              <li @click="mark()" class="cursor-pointer mx-24"><i class="iconfont icon-mark-o"></i><span>标记</span></li>
              <li @click="next()" class="cursor-pointer mx-24"><span>下一题</span><i class="iconfont icon-next"></i></li>
            </ul>
          </div>
        </div>
        </transition>
...
export default {
  ...
  data() {
    return {
      ...
      endTime: null, //考试结束时间
      time: null, //考试持续时间
      second:null,//考试剩余时间精确到秒
      flagtime:null,//用来判断当前的second是否
...
.l-bottom .final {
@apply cursor-pointer inline-block text-center bg-blue-300 w-60 rounded-xl my-5 ml-2.5 h-7 leading-7 text-white  mt-5
...
.l-bottom .item p {
  @apply mb-4 mt-2 text-black ml-2 tracking-wider
